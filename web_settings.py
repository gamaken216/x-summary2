...
HELP_TEMPLATE += """
    ...
    <div class="toc">
        ...
        <ol>
            ...
            <li><a href="#scheduler">Step 6: 毎日の自動実行</a></li>
...
HELP_TEMPLATE += """
    ...
    <div class="section" id="scheduler">
        <h2>⏰ Step 6: 毎日の自動実行</h2>
        <p><a href="/">設定画面</a>の「⏰ 自動実行スケジュール」で実行時刻を設定し、「設定を保存」を押すだけで自動登録されます。</p>

        <h3>動作の仕組み</h3>
        <ul>
            <li>Windowsタスクスケジューラに自動登録されます（手動操作不要）</li>
            <li>毎日、設定した時刻にバックグラウンドで実行されます（黒い画面は出ません）</li>
            <li>設定時刻にPCがオフだった場合、次にPCを起動した時に自動で実行されます</li>
            <li>時刻を変更したい場合は、設定画面で時刻を変えて再度「設定を保存」してください</li>
        </ul>

        <h3>自動実行を止めたい場合</h3>
        <p>Windowsの検索バーで「タスクスケジューラ」と入力して開き、「X-AutoSummary」を右クリック →「削除」してください。または <code>uninstall.bat</code> を実行すれば、タスクごと削除されます。</p>
    </div>
